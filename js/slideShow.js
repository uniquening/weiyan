(function($) {
	...
	SlideShow.prototype.createFakeImg = function() {
		var image = this.bar.image,
			alink = this.bar.alink,
			len = image.length;

		this.image.push(image[len - 1]);
		this.alink.push(alink[len - 1]);
		for (var i = 0; i < len; i++) {
			this.image.push(image[i]);
			this.alink.push(alink[i]);
		}
		this.image.push(image[0]);
		this.alink.push(alink[0]);
	...
	SlideShow.prototype.createDom = function() {
		...
		var picStr = '',
			...
			len = image.length;
		for (var i = 0; i < len; i++) {
			picStr += '<li><a href="' + alink[i] + '"><img src="' + image[i] + '" alt=""></a></li>';
			if (i < len - 2) {
		...
		$(this.bar.body)
			...
			.append(btnR).append(btnL).append(smlPic.html(smlStr));
		picBox.find('li').css('width', this.bar.width + 'px');
		// $('.sml-pic li').eq(0).addClass('active');
		$('.fake', $('#slide-show')).eq(0).addClass('fill')
	};
	//绑定事件
